  ...
  head: () => ({
    meta: [
      { title: "Security Guard Jobs UK | Careers at Aurex Security" },
      {
        name: "description",
        content:
          "Join Aurex Security. SIA-licensed security guard vacancies across the UK. Competitive pay, training and progression.",
      },
      { property: "og:title", content: "Careers — Aurex Security" },
      {
        property: "og:description",
        content: "SIA security guard jobs across the UK with Aurex Security.",
      },
...
const VACANCIES = [
  ...
  {
    role: "Construction Site Officer",
    location: "Manchester",
    type: "Full-time",
    rate: "£13.50 – £16/hr",
  },
  {
    role: "Corporate Concierge Security",
    location: "London EC2",
    type: "Full-time",
    rate: "£15 – £18/hr",
  },
...
function CareersPage() {
  ...
  const field =
    "w-full bg-background/40 border border-border focus:border-gold focus:ring-1 focus:ring-gold/40 rounded-sm px-4 py-3 text-sm outline-none transition-colors";
      ...
      <Section
        ...
        title={
          <>
            ...
            <span className="gradient-gold-text">UK's most disciplined</span> security team.
          </>
        }
        description="We invest in our officers — competitive rates, structured training and clear progression into supervisory and control-room roles."
      >
          ...
          {[
            {
              i: Briefcase,
              t: "Steady Contracts",
              d: "Long-term placements with leading UK clients.",
            },
            {
              i: ShieldCheck,
              t: "Officer Welfare",
              d: "Senior support 24/7 and quality uniform issued.",
            },
            {
              i: GraduationCap,
              t: "Training Funded",
              d: "SIA refreshers, FAW, ProtectDuty and more.",
            },
            {
              i: Heart,
              t: "Real Progression",
              d: "Promotion into supervision, control room, account management.",
            },
          ].map((b) => (
        ...
        <div className="rounded-md border border-border/60 overflow-hidden">
          {VACANCIES.map((v, i) => (
            <div
              key={v.role}
              className={`grid grid-cols-2 md:grid-cols-4 gap-4 p-5 items-center ${i !== VACANCIES.length - 1 ? "border-b border-border/60" : ""} hover:bg-surface/40 transition-colors`}
            >
              <div>
              ...
              <a
                href="#apply"
                className="justify-self-end text-xs uppercase tracking-[0.22em] text-foreground/80 hover:text-gold transition-colors"
              >
                Apply →
              </a>
            </div>
        ...
        {done ? (
          <div className="glass rounded-md p-10 text-center">
            ...
            <p className="mt-2 text-muted-foreground text-sm">
              Our recruitment team will review your details and respond within 3 working days.
            </p>
          </div>
        ) : (
          <form
            onSubmit={onSubmit}
            className="glass rounded-md p-6 md:p-10 grid gap-5 md:grid-cols-2 max-w-3xl"
          >
            <div>
            ...
            <div>
              <label className={lab}>Applying For</label>
              <select name="role" defaultValue="" className={field}>
                <option value="" disabled>
                  Select a role
                </option>
                {VACANCIES.map((v) => (
                  <option key={v.role}>{v.role}</option>
                ))}
              </select>
            </div>
            <div className="md:col-span-2">
              <label className={lab}>SIA License Held</label>
              <select name="sia" defaultValue="" className={field}>
                ...
                <option>Not yet — pursuing</option>
              </select>
            </div>
            <div className="md:col-span-2">
              <label className={lab}>Brief Message</label>
              <textarea name="message" rows={4} className={field} />
            </div>
            <button
              type="submit"
              className="md:col-span-2 inline-flex items-center justify-center gap-2 px-7 py-3.5 rounded-sm font-medium text-primary-foreground bg-[image:var(--gradient-gold)] hover:brightness-110 transition-all"
            >
              Submit Application
            </button>
