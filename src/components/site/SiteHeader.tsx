        ...
        <div className="max-w-7xl mx-auto container-px flex items-center justify-between py-4">
          <Link to="/" className="flex items-center gap-3 group">
            <div className="relative h-10 w-10 rounded-md border border-gold/30 flex items-center justify-center bg-charcoal/60 group-hover:shadow-gold-glow transition-shadow">
              <span className="font-display text-xl gradient-gold-text leading-none font-semibold">
                AX
              </span>
            </div>
          ...
          <div className="hidden lg:flex items-center gap-4">
            <Link
              to="/emergency"
              className="text-xs uppercase tracking-widest text-foreground/70 hover:text-destructive transition-colors"
            >
              Emergency
            </Link>
            <Link
              to="/contact"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-sm text-sm font-medium text-primary-foreground bg-[image:var(--gradient-gold)] hover:brightness-110 transition-all shadow-gold-glow"
            >
              Request Quote
            </Link>
          </div>
          ...
          <div className="absolute right-0 top-0 h-full w-[88%] max-w-sm bg-charcoal border-l border-gold/20 p-6 overflow-y-auto animate-fade-up">
            <div className="flex items-center justify-between mb-8">
              ...
              <button
                ...
                className="h-10 w-10 inline-flex items-center justify-center rounded-md border border-border/60"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            <nav className="flex flex-col gap-1">
              {NAV.map((n) => (
                <Link
                  ...
                  className="px-4 py-3 rounded-md border border-transparent hover:border-gold/30 text-base"
                >
                  {n.label}
                </Link>
              ))}
              <Link
                to="/emergency"
                onClick={() => setOpen(false)}
                className="px-4 py-3 rounded-md text-destructive border border-destructive/40 mt-2"
              >
                Emergency Response
              </Link>
            </nav>
            <a
              href="tel:+447723406782"
              className="mt-6 flex items-center justify-center gap-2 px-5 py-3 rounded-sm bg-[image:var(--gradient-gold)] text-primary-foreground font-medium"
            >
